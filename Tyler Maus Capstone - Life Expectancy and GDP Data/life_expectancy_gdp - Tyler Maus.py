...
bar1=sns.barplot(data=df,
                 x=df.Country,
                 y=df.GDP,
                 palette='Set1').set_title('Average GDP by County for 2000-2015')
...
plt.xticks(rotation=45)
sns.set(style = 'darkgrid')
bar2=sns.barplot(data=df,
                 x=df.Country,
                 y=df.LEABY,
                 palette='Set1').set_title('Life Expectancy at Birth (Years)')
plt.show()
...
explanation = 'They dont look similar, it appears that GDP is not the only variable that may affect life expectancy.'
print(explanation)
...
ax=sns.barplot(data=df,
               ...
               y=df.GDP,
               hue='Year',
               palette='YlOrRd').set_title('GDP in Trillions of U.S. Dollars by Country')
ax=plt.ylabel("GDP in Trillions of U.S. Dollars")
...
ax=sns.barplot(data=df,
               ...
               y=df.LEABY,
               hue='Year',
               palette='Accent').set_title('Life expectancy at birth in years')
ax=plt.ylabel("Life expectancy at birth in years")
...
explanation2 = 'Zimbabwes bars have changed the most. The bigges change has taken place from 2005 to 2015 where the average life expectancy has increased by more than 10 years.Zimbabwe had the lowest increase in GDP.In terms of life expectancy, all countries are fairly close except Zimbabwe. The United States GDP is far greater than the other countries. Though GDP does have an effect on life expectancy there are other factors contributing as Chile has a low GDP but their life expectancy is comparable. Zimbabwe has rapidly declining economy and Chinas economy has increased in the recent years.'
print(explanation2)
...
ex4 = '1)China and USA 2) Zimbabwe 3) Yes as it is a dramtic increase 4) I believe that they are easy to read and understand.'
print(ex4)
...
g3 = (g3.map(plt.plot, 'Year', 'LEABY',color='orange').add_legend())
plt.show()
g.savefig('scat.png')
...
#  2)2005-2015
#  3)Mexico
#  4)Economy,unrest,dangerous environments and access to quality resources
...
g4 = (g4.map(plt.plot, 'Year', 'GDP',color='Green').add_legend())
plt.show()
...
Lowest='Zimbabwe' Highest='Germany'
...
explanation3='Wealth has doubled in China, allowing citizens to buy items thus stimulating the economy.'
print(explanation3)
...
Please refer to link.txt. Thank you
